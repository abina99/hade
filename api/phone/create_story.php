        ...
        if (empty($user_login_data)) {
            ...
            if (!$error) {
                ...
                if (count($registration_data) > 0) {
                    ...
                    if ($last_id && is_numeric($last_id) && !empty($_POST['images'])) {
                        $true = false;
                        $files_decode = json_decode($_POST['images'], true);
                        $sources = array();
                        foreach ($files_decode as $key => $file_base64) {
                            $imageData = base64_decode($file_base64);
                            $dir       = "upload/photos/" . date('Y') . '/' . date('m');
                            $filename  = $dir . '/' . Wo_GenerateKey() . '_' . date('d') . '_' . md5(time()) . "_image.jpg";
                            $h = fopen($filename, 'w');
                            fwrite($h, $imageData);
                            fclose($h);
                            $thumb = '';
